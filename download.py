...
try:
    ...
    print('**** Could not import tqdm. Please install tqdm \
        for download progressbars! (pip install tqdm) ****')
...
def download_list(url_list):
    for url in url_list:
        ...
        with open(path, 'wb') as f:
            ...
            print('Downloading {} - {:.1f} MB'.format(path,
                                                      (total_length / 1024000)))

            for chunk in tqdm(r.iter_content(chunk_size=1024),
                              total=int(total_length / 1024) + 1, unit="KB"):
                if chunk:
                    f.write(chunk)
